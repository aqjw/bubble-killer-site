...
class QualityImprovementJob implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(): void
    {
        ...
        $mangaChapter->update(['status' => MangaChapterStatus::QualityImprovement]);

        $success = app(ProcessService::class)->processImproveQuality($mangaChapter);

        if ($success) {
            dispatch(new CreateBubbleMasksJob(mangaChapterId: $this->mangaChapterId));
        }
    }
}
